...
export function signWithKeypair(txXdr, passphrase, source) {
    const tx = new Transaction(txXdr, passphrase);
    tx.sign(source);
    return tx.toXDR();
}
export async function invoke(operation, source, sim) {
...
export async function invokeTransaction(tx, source, sim) {
    // simulate the TX
    ...
    const sim_tx_data = simulation_resp.transactionData
        .setResources(txResources.instructions() == 0 ? 0 : txResources.instructions() + 500000, txResources.readBytes(), txResources.writeBytes())
        .build();
    const assemble_tx = SorobanRpc.assembleTransaction(tx, simulation_resp);
    sim_tx_data.resourceFee(xdr.Int64.fromString((Number(sim_tx_data.resourceFee().toString()) + 100000).toString()));
    ...
    let response = await loadedConfig.rpc.sendTransaction(prepped_tx);
...
export async function createTxBuilder(source) {
    try {
        const account = await loadedConfig.rpc.getAccount(source.publicKey());
...
class DetailedSimulationError extends Error {
    simulationResp;
    constructor(message, simulationResp) {
        super(message);
